...
const SignupPage = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // The key is changed from `username` to `name` to match the state variable
      const { data } = await axios.post("http://localhost:5000/api/users/signup", {
        username: name, // <-- Corrected: Use the 'name' state variable here
        email,
        password,
      });
    ...
    <div className="flex flex-col md:flex-row h-screen font-sans">
      ...
      <div className="w-full md:w-1/2 flex items-center justify-center bg-white p-4">
        <div className="w-full max-w-sm md:max-w-md lg:max-w-lg p-6 md:p-10 shadow-xl rounded-2xl border border-gray-100">
          ...
          <p className="text-gray-500 mb-1">Join us today 👋</p>
